...
class Guide extends Base {
    ...
    _select(e) {
        ...
        this.bankAbbr = curEl.attr('data-abbr');
        this._showOrder();
    }
    ...
    _submit(e) {
        ...
        var postUrl = Config.getAjaxPrefix() + '/v1/user/pay/deposit_kubao/?access_token=' + 
            this.cookie.get('token') + '&amount=' + amount + '&bank_code=' + this.bankAbbr;
        ...
        return {
            tmpl: [
                '<% data.forEach(function(item) { %>',
                    '<li class="item J_BankItem" data-abbr="<%= item.bank_abbr %>">',
