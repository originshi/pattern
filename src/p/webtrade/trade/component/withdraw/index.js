...
export default class Wdraw extends Base {
  ...
  _bind () {
    ...
    doc.on('blur', '.J_OpenBankName', _.bind(this._vaildateOpenName, this));
  }
  ...
  _saveMony () {
    ...
    this.ajax({
      ...
      $.each(data.banks,function (index,item) {
          $('#mybanks').append($('<option/>').text(item.name).attr('value',item.code_name));
      })
  ...
  _getParams() {
        ...
        var bank_branch_name = $('.J_OpenBankName').val();
        if (getHasExtractWd() && bank_branch_name) {
          ret.bank_branch_name = bank_branch_name;
        }
