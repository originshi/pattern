...
var Toast = toast;
import InvitationCode from "../inviteCode/InvitationCode";
  ...
  _render() {
    this.getAccount().then((account) => {
      ...
      var data = {
        ...
        phoneClass: getRegisterHasReferCode() ? '' : 'margin-top'
      };
      this.broadcast('get:user:name:phone:img', data);

      this.render(tmpl, data, this.el);
      //新版推荐码
      if(getNewInvitationCode()){
        new InvitationCode({account:this.account,el:$("#InvitationCode")})
      }
      this._getImageCode();
    });
  }
