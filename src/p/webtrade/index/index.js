...
class Login extends Base {
	...
	_getToken() {
		...
		this.getToken().then(() => {
			...
			var nickname = this.cookie.get('nickname');
			var formEl = this.secondContentEl.find('form');
			this.secondContentEl.find('.J_Phone').val(phone);
			this.secondContentEl.find('.account-content').text(nickname);
	...
	_actionControls(e) {
		...
		var secondClick=curFormEl.attr("data-submit-sencond")
		...
		if ( sourceForm == 'login' || sourceForm == "first-login") {
			if(secondClick && params.password){
				this._submit(params, sourceForm, curFormEl);
				return;
			}
			if ( params.password && verifySuccess) {
				this._submit(params, sourceForm, curFormEl);
				
			}
			curFormEl.attr("data-submit-sencond",true);
			return;
		}
