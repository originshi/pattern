...
Base.extend(PageBase, Base, {
  ...
  getMargin: function(openPrice, symbol, volume, accountData) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      if ( isHasFixedMargin ) {
        var fixed_margin;
        var home_currency_fixed_margin_ratio = parseFloat(symbol.policy.home_currency_fixed_margin_ratio);
        if (home_currency_fixed_margin_ratio > 0) {
            fixed_margin = home_currency_fixed_margin_ratio * volume;
        } else {
            var fixed_margin_ratio = parseFloat(symbol.policy.fixed_margin_ratio);
            fixed_margin = fixed_margin_ratio * volume;
        }
