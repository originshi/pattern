...
Datafeeds.UDFCompatibleDatafeed = function(datafeedURL, updateFrequency, protocolVersion) {
  ...
  this._initialize();
    ...
    // cooper 2016/01/11 鐢ㄦ潵淇濆瓨鏈€鍚庝竴涓猙ar锛屼互渚垮拰鐢╯tomp鏀跺埌鐨剄uote鍚堝苟銆�
    // key鏄� symbol + "=" + resolution, value鏄� {time, close, open, high, low, volume} 鍏蜂綋鍦ㄨ繖涓〉闈㈤噷锛�
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function(symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
  ...
  function onResultReady(data) {
    ...
    that._logMessage("Symbol resolved: " + (Date.now() - resolveRequestStartTime));
        var symbol = postProcessedData.ticker;
        var tzname = postProcessedData.timezone;
        
        that.tznames[symbol] = tzname;
  ...
  .done(function (response) {
    ...
    for (var i = 0; i < barsCount; ++i) {
      ...
      bars.push(barValue);
            ...
            if (i == barsCount - 1) {
                var k = symbolInfo.ticker + "=" + resolution;
                // that 鏄� UDFCompatibleDatafeed 瀵硅薄銆�
                that._lastBars[k] = barValue;
            }
    }
...
// epoch 浠ヨ豹绉掍负鍗曚綅
function roundEpoch(epoch, resolution, tzname) {
    var  ret = epoch;
    if (resolution == '1') {
        var d = 60 * 1000;
        ret = parseInt(ret / d) * d;
        return {"start": ret, "end": ret + d};
    }
    if (resolution == '5') {
        var d = 5 * 60 * 1000;
        ret = parseInt(ret / d) * d;
    ...
    if (resolution == 'D') {
        var dt = new timezoneJS.Date(epoch, tzname);
        var offset = dt.getTimezoneOffset() * 60 * 1000; // 鏃跺尯渚垮疁閲忋€俤t.getTimezoneOffset()鏄互鍒嗛挓涓哄崟浣嶇殑銆俹ffset瑕佷互姣涓哄崟浣嶃€�
        ret = epoch - offset;
        var d = 24 * 60 * 60 * 1000;
        ret = parseInt(ret / d) * d;
        ret = ret + offset;
...
Datafeeds.UDFCompatibleDatafeed.prototype.onStompQuote = function(message, resolution) {
    // console.log("Datafeeds.UDFCompatibleDatafeed.prototype.onStompQuote");
    // console.log(message);
    var quote = JSON.parse(message.body);
    if (quote.f == "v2") {
        // symbol,askPrice,askSize,bidPrice,bidSize,lastPrice,lastSize,receiveTime
        var res = quote.d.split(",");
        quote.symbol = res[0];
        quote.askPrice = res[1];
        quote.askSize = res[2];
        quote.bidPrice = res[3];
        quote.bidSize = res[4];
        quote.lastPrice = res[5];
        quote.lastSize = res[6];
        quote.receiveTime = res[7];
        
        var parts = quote.receiveTime.match(/(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})/);
        var epoch = Date.UTC(+parts[1], parts[2]-1, +parts[3], +parts[4], +parts[5], +parts[6]);
        quote.epoch = epoch - 3600 * 8 * 1000;
    }
    // console.log(quote.symbol + " " + quote.epoch  );
    quote.subid = message.headers.subscription;
    for (var listenerGUID in this._barsPulseUpdater._subscribers) {
        if (quote.subid == listenerGUID) {
            // console.log("quote.subid == listenerGUID");
            var subrecord = this._barsPulseUpdater._subscribers[listenerGUID];
            for (var i=0; i < subrecord.listeners.length; ++i) {
                var listener = subrecord.listeners[i];
                // bar: object {time, close, open, high, low, volume}
                
                var k = quote.symbol + "=" + resolution
                var bar = this._lastBars[k];
                // var end = bar.time + 86400 * 1000;
                bar['close'] = parseFloat(bar['close']);
                bar['open'] = parseFloat(bar['open']);
                bar['low'] = parseFloat(bar['low']);
                bar['high'] = parseFloat(bar['high']);
                
                var tzname = this.tznames[quote.symbol];
                var interval = roundEpoch(bar.time, resolution, tzname);
                quote.bidPrice = parseFloat(quote.bidPrice);
                if (interval.start <= quote.epoch && interval.end > quote.epoch ) { // quote 搴旇鍦╞ar閲岄潰銆�
                    if (quote.bidPrice > bar.high) {
                        bar.high = quote.bidPrice;
                    }
                    if (quote.bidPrice < bar.low) {
                        bar.low = quote.bidPrice;
                    }
                    bar.close = quote.bidPrice;
                } else if (interval.start > quote.epoch) {// 浠庢椂闂翠笂鐪嬶紝quote 鍦ㄦ渶鍚庝竴涓猙ar鍓嶉潰锛岃繖绉嶆儏鍐典笉搴旇鍙戠敓锛岃繖涓垎鏀敤鏉ヤ互闃蹭竾涓€銆�
                    // 浠€涔堥兘涓嶇敤鍋氥€�
                } else { // quote 搴旇寮€鍚竴涓柊鐨刡ar
                    bar = { "time": roundEpoch(quote.epoch, resolution, tzname).start, 
                            "close": quote.bidPrice, 
                            "open": quote.bidPrice, 
                            "high": quote.bidPrice, 
                            "low": quote.bidPrice, 
                            "volume": 0 };
                    this._lastBars[k] = bar;
                }
                bar['close'] = parseFloat(bar['close']);
                bar['open'] = parseFloat(bar['open']);
                bar['low'] = parseFloat(bar['low']);
            ...
            // console.log("quote.subid = " + quote.subid + " listenerGUID = " + listenerGUID);
        }
    }
}

Datafeeds.UDFCompatibleDatafeed.prototype.set_groupname = function(gn) {
    this.groupname = gn;
}
...
Datafeeds.UDFCompatibleDatafeed.prototype.subscribeBars = function(symbolInfo, resolution, onRealtimeCallback, listenerGUID) {
  this._barsPulseUpdater.subscribeDataListener(symbolInfo, resolution, onRealtimeCallback, listenerGUID);
    ...
    if (this.stompHelper) {
        ...
        var dest = "quote." + this.groupname + "." + symbol + "?format=v2&throttle=0"
        // console.log("dest: " + dest);
        if (this.stompHelper.connected()) {
            var self = this;
            var sub = this.stompHelper.subscribe(listenerGUID, dest, 
                                                    function(message) {self.onStompQuote(message, resolution);}
                                                  );
        } else {
            var login = "gooduser";
            var passcode= "passwd1";
            var host = "localhost";
            // console.log(this.stompHelper.stomp);
            var that = this;
            this.stompHelper.connect(login, passcode, 
                function() {
                    // console.log("stomp connected");
                    //console.log("client.connected: " + qh.connected()); 
                    //var sub = that.stompHelper.subscribe(listenerGUID, dest, 
                    // this.onStompQuote 
                    //onmessage
                    //);
                }, 
...
Datafeeds.UDFCompatibleDatafeed.prototype.unsubscribeBars = function(listenerGUID) {
  this._barsPulseUpdater.unsubscribeDataListener(listenerGUID);
    if (this.stompHelper) {
        this.stompHelper.unsubscribe(listenerGUID);
    }
};
